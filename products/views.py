    ...
    if request.user.is_authenticated:
        ...
        if sort_by != None:
            if sort_by == 'date_new':
                ...
                products_list = products_list.order_by('product__-create_at')
        ...
        if request.method == 'POST':
            ...
            min = float(request.POST.get('min'))
            max = float(request.POST.get('max'))
            products_list = products_list.filter(product__price__gte=min, product__price__lte=max)
            if star:
                active_stars.append(f'star{star}')
                showing_active_filters.append(f'Star: {star}')
                if star > 1:
                    products_list = products_list.filter(product__stars__gte=star, product__stars__lt=star+1)
                else:
                    products_list = products_list.filter(product__stars__gte=star-1, product__stars__lt=star+1)
            ...
            for i in brands:
                if request.POST.get(f'brand_{i}'):
                    ...
                    showing_active_filters.append(f'Brand: {i}')
            if active_brands:
                products_list = products_list.filter(product__brand__in=active_brands)
            ...
            for i in categories:
                ...
                for c in product_categories:
                    if request.POST.get(f'{i}_{c.name}'):
                        ...
                        product_categories_for_filter.append(c.name)
            if categories_for_filter:
                products_list = products_list.filter(product__productcategory__category__in=categories_for_filter, product__productcategory__name__in=product_categories_for_filter).distinct()
            
            search = request.POST.get('hidden_search')
            if search:
                products_list = products_list.filter(product__name__icontains=search)
                active_search = search
        ...
        for i in products_list:
            ...
            sale = None
            if ProductSale.objects.filter(product=i.product).exists():
                sale = get_product_sale(i)
            ...
            if rating_list:
                ...
                for s in rating_list:
                    rating += s.stars
                rating = round(rating / len(rating_list), 1)
                products.append([i.product, photo, rating, len(orders), saved, sale])
            else:
                products.append([i.product, photo, 0.0, len(orders), saved, sale])
